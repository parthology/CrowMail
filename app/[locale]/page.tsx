...
function MainContent() {
  ...
  // One-click temporary mailbox creation (manually triggered by user)
  const handleQuickCreate = async () => {
    if (isCreatingAccount) return
    setIsCreatingAccount(true)

    const maxAttempts = 5
    const domain = "crowmail.sbs"
    ...
    for (let attempt = 0; attempt < maxAttempts; attempt++) {
      ...
      try {
        // One-click temporary mailboxes do not expire by default
        ...
        // Only retry with a new username when the address is taken (very low probability)
        if (isAddressTaken && attempt < maxAttempts - 1) {
          continue
        }

        // Show any other errors directly to the user
        console.error("One-click temporary mailbox creation failed:", error)
        toast({
  ...
  const handleSidebarItemClick = (item: string) => {
    ...
    if (item === "github") {
      window.open("https://github.com/parthology/CrowMail", "_blank", "noopener,noreferrer")
      return
    }

    if (item === "discord") {
      window.open("https://discord.gg/crowmail", "_blank", "noopener,noreferrer")
      return
    }

    if (item === "telegram") {
      window.open("https://t.me/crowmails", "_blank", "noopener,noreferrer")
      return
    }
        ...
        <div className="flex-1 flex flex-col overflow-hidden">
          ...
          {isMobile && (
            <div className="flex items-center justify-between p-4 border-b border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-900">
              ...
              <div className="flex items-center space-x-2">
                ...
                <span className="font-semibold text-lg text-gray-800 dark:text-white">CrowMail</span>
              </div>
              <div className="w-8" />
            </div>
          )}
          ...
          <main className="flex-1 overflow-y-auto">
            <div className="h-full flex flex-col">
              ...
              {/* Logged-out landing page: feature cards + ads (real content to satisfy AdSense requirements) */}
              {(!isAuthenticated || !currentAccount) && (
                <FeatureCards />
              )}
            </div>
